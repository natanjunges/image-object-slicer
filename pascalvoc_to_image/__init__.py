...
def get_pascal_file(path):
    return [{ 'base': path, 'filename': file, 'path': os.path.join(path, file)} for file in tqdm(os.listdir(path), desc="Loading pascal files", ascii=True) if file.endswith('.xml')]

def parse_pascal_file(file):
...
def pascal_to_images(pascal_data, save_path):
    for item in tqdm(pascal_data, ascii=True, desc="Creating images from pascal data"):
        pascalvoc_to_image(item, save_path)


if __name__ == '__main__':
    pascal_files = get_pascal_file(args.pascalDir)
    parsed_pascal_files = parse_pascal_files(pascal_files)
    make_dir(args.saveDir)
    create_label_dirs(parsed_pascal_files.get('labels'), args.saveDir)
    pascal_to_images(parsed_pascal_files.get('pascal_data'), args.saveDir)
